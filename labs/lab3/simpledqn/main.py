...
class DQN(object):
    ...
    def compute_q_learning_loss(self, l_obs, l_act, l_rew, l_next_obs, l_done):
        ...
        q_predictions = C.functions.select_item(q_predictions_matrix, l_act)

        qt_predictions_matrix = self._qt.forward(l_next_obs).data
        qt_max_predictions = C.functions.max(qt_predictions_matrix, 1)
    ...
    def compute_double_q_learning_loss(self, l_obs, l_act, l_rew, l_next_obs, l_done):
        ...
        qt_predictions_matrix = self._qt.forward(l_next_obs)

        double_q_next_actions_matrix = self._q.forward(l_next_obs)
        double_q_max = C.functions.argmax(double_q_next_actions_matrix, 1)

        qt_max_predictions = C.functions.select_item(qt_predictions_matrix, double_q_max)
